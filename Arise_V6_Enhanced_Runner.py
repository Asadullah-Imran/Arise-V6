...
"""
================================================================================
  🧬 ARISE-V6 Component Ablation Enhanced Runner (Silhouette Preserving)
================================================================================

  Enhanced runner designed to maintain and elevate Silhouette scores throughout
  both Pre-training (Stage 1) and DEC Fine-Tuning (Stage 2).

  Key Enhancements over Baseline Runner:
  ------------------------------------------------------------------------------
  1. Hypersphere Embedding Normalization (L2 norm) for clustering & Silhouette.
  2. Decoupled, Gentle DEC Learning Rate (default 0.1x of base lr).
  3. Controlled KL divergence weighting (lambda_kl tuning) to anchor geometry.
  4. Periodic Target Distribution (P) updates to stabilize cluster centroids.
  5. Detailed Dual Tracking (Normalized & Raw Silhouette, Peak ARI, Terminal).
================================================================================
"""
...
# Add sub-folders to path dynamically across local, Colab, and Kaggle
search_roots = [
    os.path.dirname(os.path.abspath(__file__)),
    os.getcwd(),
    os.path.join(os.path.dirname(os.path.abspath(__file__)), 'Arise-V6'),
    os.path.join(os.getcwd(), 'Arise-V6'),
    '/content/Arise-V6',
    '/content',
    '/kaggle/working/Arise-V6',
    '/kaggle/working'
]

subfolders = ['fusion-ablation', 'loss-ablation', 'encoder-ablation', 'contrastive-ablation']
...
from loss_models import LOSS_MODEL_REGISTRY, compute_target_distribution, consensus_distribution_loss
...
# ----------------------------------------------------------------------
# 4. EVALUATION METRICS & EMBEDDING NORMALIZATION
# ----------------------------------------------------------------------
def compute_all_metrics(y_true: np.ndarray, y_pred: np.ndarray, embeddings: np.ndarray, use_norm_sil: bool = True) -> Dict[str, float]:
    y_true_str = np.asarray(y_true).astype(str)
    y_pred_str = np.asarray(y_pred).astype(str)
    
    # 1. Standard biological ground-truth alignment metrics
    metrics = {
    ...
    # 2. Raw Euclidean Silhouette
    try:
        metrics['Silhouette_Raw'] = float(silhouette_score(embeddings, y_pred_str))
    except Exception:
        metrics['Silhouette_Raw'] = 0.0

    # 3. Hypersphere Unit-Normalized Silhouette (prevents scaling drift)
    norm_emb = embeddings / (np.linalg.norm(embeddings, axis=1, keepdims=True) + 1e-8)
    try:
        metrics['Silhouette_Norm'] = float(silhouette_score(norm_emb, y_pred_str))
    except Exception:
        metrics['Silhouette_Norm'] = metrics['Silhouette_Raw']

    # Primary Silhouette metric (selected based on configuration)
    metrics['Silhouette'] = metrics['Silhouette_Norm'] if use_norm_sil else metrics['Silhouette_Raw']
    return metrics


def run_kmeans_clustering(embeddings: np.ndarray, num_clusters: int, seed: int = 42, normalize_input: bool = True) -> np.ndarray:
    if normalize_input:
        emb_eval = embeddings / (np.linalg.norm(embeddings, axis=1, keepdims=True) + 1e-8)
    else:
        emb_eval = embeddings
    kmeans = KMeans(n_clusters=num_clusters, n_init=10, random_state=seed)
    return kmeans.fit_predict(emb_eval)


# ----------------------------------------------------------------------
# 5. ENHANCED TWO-STAGE DEC ABLATION RUNNER (SILHOUETTE PRESERVING)
# ----------------------------------------------------------------------
def train_and_evaluate_variant_enhanced(variant: str, model, graph_data, ground_truth, num_clusters: int, args, seed: int):
    start_time = time.time()
    
    # Configure custom KL divergence weight if provided
    if hasattr(model, 'lambda_kl') and args.lambda_kl is not None:
        model.lambda_kl = args.lambda_kl

    # Base Stage 1 Optimizer
    ...
    for epoch in range(pretrain_epochs):
        ...
        pred_labels = run_kmeans_clustering(emb, num_clusters, seed=seed, normalize_input=args.norm_clustering)
        metrics = compute_all_metrics(ground_truth, pred_labels, emb, use_norm_sil=args.norm_silhouette)
        ...
        if (epoch + 1) % 50 == 0 or epoch == 0 or epoch == pretrain_epochs - 1:
            print(f"[{variant} | Seed {seed}] Pre-train Epoch {epoch+1:3d}/{pretrain_epochs} | Loss: {loss.item():.4f} | ARI: {ari:.4f} | NMI: {nmi:.4f} | Sil(Norm): {sil:.4f}")

    # ------------------------------------------------------------------
    # Stage 2 Trackers (DEC Fine-Tuning with Preserved Geometry)
    # ------------------------------------------------------------------
    best_dec_sil = -1.0
    ...
    if finetune_epochs > 0:
        ...
        # Initialize cluster centers on best pretrain representations
        kmeans = KMeans(n_clusters=num_clusters, n_init=10, random_state=seed)
        if args.norm_clustering:
            init_emb = pretrain_best_embeddings / (np.linalg.norm(pretrain_best_embeddings, axis=1, keepdims=True) + 1e-8)
        else:
            init_emb = pretrain_best_embeddings
        kmeans.fit(init_emb)
        model.set_cluster_centers(kmeans.cluster_centers_)

        # Enhanced DEC Optimizer: gentle fine-tuning rate to prevent manifold collapse
        dec_lr = args.lr * args.dec_lr_scale
        optimizer_dec = torch.optim.Adam(model.parameters(), lr=dec_lr, weight_decay=args.dec_weight_decay)

        # Precompute initial target distribution P
        with torch.no_grad():
            init_out = model(graph_data, compute_q=True)
            cached_target_P = compute_target_distribution(init_out['q_list'][0])

        model.train()
        for epoch in range(finetune_epochs):
            # Periodic update of target distribution P to stabilize cluster centroids
            if args.p_update_interval > 0 and (epoch % args.p_update_interval == 0):
                with torch.no_grad():
                    q_cur = model(graph_data, compute_q=True)['q_list'][0]
                    cached_target_P = compute_target_distribution(q_cur)

            optimizer_dec.zero_grad()
            outputs = model(graph_data, compute_q=True)
            
            # Compute loss with stabilized target P if periodic updates enabled
            if args.p_update_interval > 0 and outputs['q_list'] is not None:
                # Custom Stage 2 with cached P
                loss, loss_dict = model.compute_loss(graph_data, outputs, stage=1)
                l_kl = consensus_distribution_loss(outputs['q_list'], cached_target_P)
                lambda_kl_val = getattr(model, 'lambda_kl', 0.02)
                loss = loss + lambda_kl_val * l_kl
            ...
            if (epoch + 1) % 50 == 0 or epoch == 0 or epoch == finetune_epochs - 1:
                print(f"[{variant} | Seed {seed}] DEC Fine-tune Epoch {epoch+1:3d}/{finetune_epochs} | Loss: {loss.item():.4f} | KL: {loss_dict.get('loss_kl', 0.0):.4f} | ARI: {ari:.4f} | NMI: {nmi:.4f} | Sil(Norm): {sil:.4f}")
    ...
    final_metrics = compute_all_metrics(ground_truth, final_lab, final_emb, use_norm_sil=args.norm_silhouette)
    ...
    header_title = f"📊 ENHANCED SEED SUMMARY: {variant} | Seed: {seed} | Dataset: {getattr(args, 'current_ds_name', 'Benchmark')}"
    print("\n" + "┌" + "─" * 86 + "┐")
...
def main():
    parser = argparse.ArgumentParser(description="ARISE-V6 Enhanced Silhouette-Preserving Ablation Runner")
    ...
    parser.add_argument('--lr', type=float, default=1e-3, help="Stage 1 Pre-training Learning Rate")
    
    # Enhanced Silhouette & DEC Stabilization Controls
    parser.add_argument('--dec_lr_scale', type=float, default=0.1, help="Scale factor for Stage 2 DEC fine-tuning learning rate (default: 0.1x)")
    parser.add_argument('--dec_weight_decay', type=float, default=1e-4, help="Weight decay during DEC fine-tuning")
    parser.add_argument('--lambda_kl', type=float, default=0.02, help="DEC consensus KL loss coefficient (default: 0.02, anchors geometric structure)")
    parser.add_argument('--p_update_interval', type=int, default=5, help="Target distribution P update frequency in epochs (default: 5)")
    parser.add_argument('--norm_silhouette', action='store_true', default=True, help="Compute Silhouette on hypersphere unit-normalized embeddings")
    parser.add_argument('--norm_clustering', action='store_true', default=True, help="Apply L2 normalization prior to KMeans cluster evaluation")
    
    parser.add_argument('--device', type=str, default='cuda' if torch.cuda.is_available() else 'cpu')
    parser.add_argument('--out_dir', type=str, default='results_v6_enhanced')

    args = parser.parse_args()
    os.makedirs(args.out_dir, exist_ok=True)

    # Enable Dual Logging (Console + Log file)
    log_path = os.path.join(args.out_dir, "enhanced_ablation_suite.log")
    ...
    print("=" * 88)
    print(f"🧬 STARTING ARISE-V6 ENHANCED RUNNER | Track={args.track} | Epochs={args.epochs} (Pre={args.pretrain_epochs}, DEC={args.finetune_epochs})")
    print(f"🎯 Variants ({len(variants_to_run)}): {variants_to_run}")
    print(f"🛡️  DEC Stabilizers: DEC_LR_Scale={args.dec_lr_scale}x | Lambda_KL={args.lambda_kl} | P_Interval={args.p_update_interval}ep")
    ...
    for ds_idx in dataset_indices:
        ...
        for variant in variants_to_run:
            ...
            for seed in args.seeds:
                ...
                metrics, final_emb, final_labels = train_and_evaluate_variant_enhanced(
    ...
    out_csv = os.path.join(args.out_dir, "arise_v6_enhanced_results.csv")
    ...
    print("\n" + "=" * 88)
    print(f"✅ Enhanced Ablation Suite Completed! Full Results exported to: {out_csv}")
    ...
    if not df.empty:
        ...
        print("\n🏆 Consolidated V6 Enhanced Performance Summary (Mean ± Std):")
        print(summary)
